  ...
  requireLogin(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(err.error || 'Erro ao buscar chaves');
      }
      ...
      // Monta o select com as chaves recebidas
      const htmlForm = `
        <label>Chave Pix:<br/>
          <select id="inputChave" style="width:100%; padding:8px; margin-top:5px; border-radius:4px; border:none;">
            <option value="">Selecione uma chave...</option>
      ...
      modal.querySelector('h2').textContent = 'Receber Pix';
      modal.querySelector('div:nth-child(2)').innerHTML = htmlForm;

      // Footer com botões
      let footer = modal.querySelector('.footerPix');
      if (!footer) {
        footer = document.createElement('div');
        footer.className = 'footerPix';
        Object.assign(footer.style, { marginTop: '20px', textAlign: 'right' });
        footer.innerHTML = `
          <button id="btnCancelar" style="background: #555; color: #fff; border:none; padding: 8px 16px; margin-right: 10px; border-radius: 6px; cursor: pointer;">Cancelar</button>
          <button id="btnConfirmar" style="background: #ff6600; color: #fff; border:none; padding: 8px 16px; border-radius: 6px; cursor: pointer;">Confirmar</button>
        `;
      ...
      // Remove listeners antigos do botão confirmar
      const oldBtnConfirmar = document.getElementById('btnConfirmar');
      const newBtnConfirmar = oldBtnConfirmar.cloneNode(true);
      oldBtnConfirmar.parentNode.replaceChild(newBtnConfirmar, oldBtnConfirmar);

      newBtnConfirmar.onclick = async () => {
        const chave = document.getElementById('inputChave').value.trim();
        const valorRaw = document.getElementById('inputValor').value;
        const valor = valorRaw ? parseFloat(valorRaw) : undefined;
        const descricao = document.getElementById('inputDescricao').value.trim() || undefined;
